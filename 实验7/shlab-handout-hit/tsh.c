...
int builtin_cmd(char **argv)
{
    ...
    if (first_arg == NULL)//第一个元素是不是空，是空直接返回
    {
        return 1;
    }
    if (!strcmp(first_arg, "quit"))//如果是quit命令，直接退出。
    {
        exit(0);
    }
    else if (!strcmp(first_arg, "jobs"))//如果是jobs命令，就输出当前的joblist。
    {
        listjobs(jobs);
        return 1;
    }
    else if (!strcmp(first_arg, "bg") || !strcmp(first_arg, "fg"))      //如果是bg或是fg，就执行do_bgfg
    {
        do_bgfg(argv);
        return 1;
    }
 ...
 */
void waitfg(pid_t pid)//显式等待前台进程终止
{
...
void sigchld_handler(int sig)
{
    ...
    {
        if(WIFEXITED(status))//处理终止进程
        {
            deletejob(jobs,pid);
        }
        if(WIFSIGNALED(status))//处理收到外部信号而强制终止的进程
        {
            job=getjobpid(jobs,pid);
            printf("Job [%d] (%d) terminated by signal %d\n",job->jid,pid,WTERMSIG(status));
            deletejob(jobs,pid);
        }
        if(WIFSTOPPED(status))//处理收到外部信号而停止的进程
        {
            job=getjobpid(jobs,pid);
...
void sigint_handler(int sig)
{
   int olderrno = errno;
	...
	if (sigemptyset(&prev_one) < 0)//检查返回值
		unix_error("sigemptyset error");
	if (sigfillset(&mask_all) < 0)//检查返回值
		unix_error("sigfillset error");
	if (sigprocmask(SIG_BLOCK, &mask_all, &prev_one) < 0)//检查返回值
		unix_error("sigprocmask error");
	pid_t pid = fgpid(jobs);
	if (sigprocmask(SIG_SETMASK, &prev_one, NULL) < 0)//检查返回值
		unix_error("sigprocmask error");
	if (sig == SIGINT && pid != 0)
		kill(-pid, SIGINT);//发送信号给进程
	errno = olderrno;
	return;
}
